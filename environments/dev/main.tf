...
###################
module "dev_vpc" {
  source = "git::https://github.com/dre8210/terraform-aws-modules.git//modules/vpc?ref=0.15.0"
...
module "dev_security_group" {
  source = "git::https://github.com/dre8210/terraform-aws-modules.git//modules/security-group?ref=0.15.0"
  ...
  depends_on = [module.dev_vpc, aws_security_group.dev_alb_sg]
}

#WEBSERVER - EC2 INSTANCES
#############################
module "dev_webserver" {
  source = "git::https://github.com/dre8210/terraform-aws-modules.git//modules/ec2?ref=0.15.0"

  instance_config = {
    public_instance = {
      ami_id                 = var.ec2_public.ami_id
      instance_type          = var.ec2_public.instance_type
      subnet_id              = module.dev_vpc.public_subnets_ids[0]
      vpc_security_group_ids = [module.dev_security_group.public_security_group_id]
...
EOF
  ...
  depends_on = [module.dev_security_group, module.dev_vpc]
}
...
resource "aws_db_subnet_group" "dev_db_subnet_group" {
  ...
  subnet_ids = [module.dev_vpc.private_subnets_ids[0], module.dev_vpc.private_subnets_ids[1]]
...
####################################
resource "aws_lb_target_group" "dev_app_tg" {
...
resource "aws_lb" "dev_app_alb" {
  name               = "dev-app-alb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.dev_alb_sg.id]
  subnets            = [module.dev_vpc.public_subnets_ids[0], module.dev_vpc.public_subnets_ids[1]]
  ...
  depends_on = [aws_security_group.dev_alb_sg, module.dev_vpc]
}

resource "aws_lb_listener" "http_listener" {
  load_balancer_arn = aws_lb.dev_app_alb.arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.dev_app_tg.arn
  }

  depends_on = [aws_lb.dev_app_alb, aws_lb_target_group.dev_app_tg]
}

resource "aws_lb_target_group_attachment" "dev_app_tg_attachment" {
  target_group_arn = aws_lb_target_group.dev_app_tg.arn
  target_id        = module.dev_webserver.instance_id[0]
  port             = 80

  depends_on = [module.dev_webserver, aws_lb_target_group.dev_app_tg]
}
